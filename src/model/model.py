...
class Encoder(nn.Module):
    """Projects keypoint features to model dimension, adds positional encoding,
    and runs Transformer encoder layers."""
...
class Decoder(nn.Module):
    ...
    def __init__(self, hidden_dim: int, num_layers: int, dropout: float, vocab_size: int, nhead: int = 8):
        super().__init__()
        # vocab_size is now passed explicitly — no hardcoded 30522
...
class SignLanguageTransformer(nn.Module):
    ...
    def __init__(self, config: dict):
        ...
        # vocab_size comes from config (set after running build_vocab.py)
